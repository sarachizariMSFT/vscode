...
describe('Configurations', () => {
	...
	it('settings in code should match package.json', () => {
		...
		const otherPublicKeys = (Object.values(ConfigKey).filter(key => key !== ConfigKey.TeamInternal && key !== ConfigKey.Shared && key !== ConfigKey.Advanced && key !== ConfigKey.Deprecated && key !== ConfigKey.Governance) as Config<any>[]).map(setting => setting.fullyQualifiedId);
